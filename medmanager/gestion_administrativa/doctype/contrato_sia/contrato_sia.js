...
currentDocTypeNameFrm = 'Contrato SIA';

frappe.ui.form.on('Contrato SIA', {
	onload: function(frm) {
		currentNivel = 'EMPRESA';
		currentDocTypeNameFrm = 'Contrato SIA';
		...
		cur_frm.set_query("servicio", "partidas", function (doc, cdt, cdn) {
			return {
				filters: [
					...
					["Catalogo de Servicios", "area", "=", "SIA"]
				]
			};
		});
	},
	...
	boton_new: function(frm) {
		...
		var d = frappe.model.add_child(frm.doc, "Partida SIA", "subpartidas", 0);
...
frappe.ui.form.on('Partida SIA', {
	...
	subpartidas_add: function(frm, cdt, cdn) {
		...
		//frm.set_df_property('Partida SIA','servicio', 'read_only', 1)
